...
public class Common {
    ...
    public static RestaurantWeek ParseWeekFromDocument(XWPFDocument document){
        // use these to determine if there's new data or not
        POIXMLProperties props = document.getProperties();
        Date created = props.getCoreProperties().getCreated();
        Date lastSave = props.getCoreProperties().getModified();
        ...
        text = text.trim().replaceAll("\r+", "\t").replaceAll(" +", " ").replaceAll("\t{2,}|\t+ +\t+", "\t").replaceAll("\n", "\t").replaceAll("\n\n", "\t").replaceAll("\n\n\n", "\t");
